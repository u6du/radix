...
// be terminated.
type WalkFn func(s []byte, v [32]byte) bool
...
func (t *Tree) Add(s []byte, v [32]byte) ([32]byte, bool) {
	t.rw.Lock()
	defer t.rw.Unlock()
	var parent *node
	n := t.root
...
func (t *Tree) Delete(s []byte) ([32]byte, bool) {
	t.rw.Lock()
	defer t.rw.Unlock()
	var parent *node
	var label byte
...
func (t *Tree) deletePrefix(parent, n *node, prefix []byte) uint {
	...
	if len(prefix) == 0 {
		...
		recursiveWalk(n, func(s []byte, v [32]byte) bool {
			subTreeSize++
			return false
		})
		if n.isLeaf() {
...
func (t *Tree) Get(s []byte) ([32]byte, bool) {
	t.rw.RLock()
	defer t.rw.RUnlock()
	n := t.root
	search := s
...
func (t *Tree) LongestPrefix(s []byte) ([]byte, [32]byte, bool) {
